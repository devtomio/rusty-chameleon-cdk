...
const sendRequest = async (
  commands: any,
  parameters: { id: string; token: string },
) => {
  const config: any = await ssmCredentials([parameters.id, parameters.token]);
...
await sendRequest(commands, {
  id: "CHAMELEON_BOT_APP_ID",
  token: "CHAMELEON_BOT_TOKEN",
});
